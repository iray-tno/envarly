...
npm run render:gif       # -> out/walkthrough.gif (for README/LP autoplay)
```

`capture-video.mjs` is the single source of truth for the narrative — it records both the frame sequence (dense bursts around each interaction, sparse during holds — full 30fps capture isn't achievable over CDP round-trips and isn't needed, since most of the motion is the app's own CSS transitions) and the timed captions into `remotion/public/frames/manifest.json`. `remotion/Walkthrough.jsx` just renders whatever that manifest says; to change what the video shows or says, edit the interaction script in `capture-video.mjs` and re-run — never edit the Remotion composition's timing by hand.

`remotion/public/` (frames, manifest, the titlebar copy) is gitignored — it's fully regenerated by `capture:video`, not committed.
