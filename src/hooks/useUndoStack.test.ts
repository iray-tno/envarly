...
describe("useUndoStack", () => {
  ...
  it("executes operations in the correct LIFO / FIFO order", () => {
    const { result } = renderHook(() => useUndoStack());
    const log: string[] = [];
    act(() => { result.current.push({ undo: () => log.push("u1"), redo: () => log.push("r1") }); });
    act(() => { result.current.push({ undo: () => log.push("u2"), redo: () => log.push("r2") }); });
    act(() => { result.current.undo(); }); // u2 (most recent first)
    act(() => { result.current.undo(); }); // u1
    act(() => { result.current.redo(); }); // r1 (oldest first)
