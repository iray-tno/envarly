    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-start gap-2">
        <div className="flex-1 min-w-0">
          <p className="text-[10px] text-dim uppercase tracking-wide mb-0.5">{t("snapshot_compare.heading")}</p>
          <p className="text-sm font-semibold text-fg truncate">{from.label}</p>
          <p className="text-[11px] text-dim">{t("snapshot_compare.to_label", { label: to.label })}</p>
        </div>
        <Button variant="ghost" size="sm" onClick={onBack}>{t("snapshot_compare.back")}</Button>
      </div>
