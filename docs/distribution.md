...
- Automated: publishing a non-prerelease GitHub Release triggers this workflow, which runs `wingetcreate update` against the release's MSI asset and opens the PR against `microsoft/winget-pkgs` directly. The workflow also passes the GitHub release's own body as `--release-notes` — winget-pkgs' validation flags a version update as inconsistent if `ReleaseNotes` was present on the last submitted version and goes missing on the next one, and `wingetcreate` won't carry that field forward on its own. Nothing to do manually unless the workflow fails (check the Actions log — a missing/renamed MSI asset or an expired `WINGET_TOKEN` are the likely causes).
